...
function locationRow(inp) {
    ...
    if (!inp.site || inp.lat == null || inp.lon == null) {
        return { key: "location", label: "Where you are", held: false,
            given: "Nothing yet.",
            earned: [],
            next: "A ZIP or an address brings your climate card: frost dates, your hardiness zone, and the "
...
function addressRow(inp) {
    ...
    if (!inp.address && !inp.anchored) {
        return { key: "address", label: "Your address", held: false,
            ...
            earned: [],
            next: "An address brings the satellite view, so you trace beds on your actual yard instead of "
...
function groundRow(inp) {
    ...
    if (inp.beds.length === 0) {
        return { key: "ground", label: "Your growing areas", held: false,
            given: "Nothing yet.",
            earned: [],
            next: "Two numbers - or one outline traced on the map - brings the fit verdict: whether a plant "
...
function seasonsRow(inp) {
    ...
    if (!held) {
        return { key: "seasons", label: "What you have grown", held: false,
            given: "Nothing yet.",
            earned: [],
            next: "Logging what you plant brings the rotation check - a family that should not follow "
    ...
    if (inp.site) {
        ...
        if (cal.spring.calibrated || cal.fall.calibrated) {
            earned.push("Your own frost dates. Enough seasons are logged that they supersede the general "
                + "estimate - the calendar plans by your ground now, not by the nearest station.");
        }
        else {
            ...
            next = `${plural(left, "more logged season")} of frosts and your own dates supersede the `
                + "general estimate, for every sow date on the calendar.";
        }
    }
    const parts = [];
...
function plantsRow(inp) {
    ...
    if (inp.userSpecies === 0) {
        return { key: "plants", label: "Plants you added", held: false,
            ...
            earned: [],
            next: "Adding a variety the app does not carry earns it a place in your beds and your calendar. "
...
function accountRow(inp) {
    if (!inp.signedIn) {
        return { key: "account", label: "Your account", held: false,
            ...
            earned: [],
            next: "An account brings keeping: the same ledger on your phone and your laptop, and a backup "
